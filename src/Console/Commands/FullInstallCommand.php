...
class FullInstallCommand extends Command
{
    ...
    private function buildSteps(array $cfg): array
    {
        ...
        // Production optimization runs LAST — preload.php must exist before
        // FPM restarts with opcache.preload pointing at it, and the route/
        // config caches should reflect the fully-installed app.
        if ($cfg['optimize'] ?? true) {
            $steps['Optimizing for production'] = fn() => $this->optimizeForProduction($cfg);
        }
    ...
    /**
     * Wire the v0.5.0 production performance stack:
     *   1. authoritative composer classmap
     *   2. `php sofy optimize` → route cache + config cache + preload.php
     *   3. an opcache drop-in (99-sofy.ini) enabling preload + JIT
     *   4. restart FPM so preload engages
     *
     * Order matters: preload.php must exist (step 2) BEFORE FPM restarts
     * pointing opcache.preload at it (step 4), or FPM logs a fatal preload
     * error on startup.
     */
    private function optimizeForProduction(array $cfg): bool
    {
        $phpV    = $cfg['php'];
        $php     = trim((string) shell_exec('which php')) ?: 'php';
        $sofy    = base_path('sofy');
        $preload = base_path('bootstrap/cache/preload.php');
        $webUser = 'www-data';

        // 1 + 2 — build the caches. Run `optimize` as the web user so the
        // generated cache files are owned by FPM's user, not root.
        $runAsWeb = static function (string $cmd) use ($webUser): string {
            return "runuser -u $webUser -- $cmd 2>/dev/null || sudo -u $webUser $cmd 2>/dev/null || $cmd";
        };

        $ok = $this->runAll([
            'composer dump-autoload --optimize --classmap-authoritative --no-dev --working-dir=' . escapeshellarg(base_path()) . ' 2>/dev/null || true',
            $runAsWeb("$php $sofy optimize"),
        ]);
        if (!$ok) {
        ...
        $iniDir = is_dir("/etc/php/$phpV/fpm/conf.d") ? "/etc/php/$phpV/fpm/conf.d"
            : (is_dir('/etc/php.d') ? '/etc/php.d' : "/etc/php/$phpV/fpm/conf.d");
        @mkdir($iniDir, 0755, true);
        $iniFile = "$iniDir/99-sofy.ini";

        $ini = <<<INI
        ; Generated by php sofy full-install — Sofy production opcache profile.
        ; Re-run `php sofy optimize` and restart php-fpm after each deploy.
        opcache.enable=1
        ...
        ; Preload the whole framework into shared memory once at master start.
        opcache.preload=$preload
        opcache.preload_user=$webUser

        ; Production: stop stat()-ing every file each request. You MUST restart
        ; php-fpm after deploying new code, or changes won't take effect.
        ...
        if (file_put_contents($iniFile, $ini) === false) {
            $this->warn("  Could not write $iniFile — skipping opcache profile.");
            return $ok;
        }
        $this->info("opcache profile written → $iniFile");

        // 4 — restart FPM so preload engages (reload is not enough for preload).
        $this->runAll(["systemctl restart php$phpV-fpm"]);

        $this->info('Production optimization complete.');
        $this->comment('  Reminder: after each deploy run `php sofy optimize` then');
        $this->comment("  `systemctl restart php$phpV-fpm` (validate_timestamps=0 needs it).");
        return true;
    }
